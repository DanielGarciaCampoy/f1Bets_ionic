...
export class HttpClientNativeProvider extends HttpClientProvider {
    ...
    getContentType(response: HTTPResponse){
        if(response.headers){
    ...
    public get<T>(
        ...
        var _params: Record<string, string> = {}; // Cambiado el tipo a Record<string, string>
        ...
        return from(this.http.get(url, _params, headers)).pipe(
            map((httpResponse) => {
                ...
                if(contentType && contentType.includes('application/json')) // cambio de (contentType.includes('application/json'))
